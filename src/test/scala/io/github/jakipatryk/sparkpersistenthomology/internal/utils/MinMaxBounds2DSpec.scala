...
class MinMaxBounds2DSpec extends AnyFlatSpec with BeforeAndAfterAll {
  ...
  "boundsPerKeyFromRDD" should "return bounds for each key in the RDD if all values are positive" in {
    ...
    assert(
      result(1) == MinMaxBounds2D(SingleDimMinMaxBound(1.2, 2.0), SingleDimMinMaxBound(1.1, 8.0))
    )
    assert(
      result(2) == MinMaxBounds2D(SingleDimMinMaxBound(0.0, 0.0), SingleDimMinMaxBound(1.0, 1.0))
    )
  }
  ...
  "boundsPerKeyFromRDD" should "return bounds for each key in the RDD if all values are negative" in {
    ...
    assert(result contains 2)
    assert(
      result(1) == MinMaxBounds2D(
        SingleDimMinMaxBound(-2.0, -1.2),
        SingleDimMinMaxBound(-8.0, -1.1)
      )
    )
    assert(
      result(2) == MinMaxBounds2D(SingleDimMinMaxBound(0.0, 0.0), SingleDimMinMaxBound(-1.0, -1.0))
  ...
  "boundsPerKeyFromRDD" should "return bounds for each key in the RDD if values are both positive and negative" in {
    val data =
      List((1, (2.0, -1.1)), (1, (-1.2, 3.0)), (1, (-1.8, 8.0)), (2, (0.0, -1.0)), (2, (0.0, 18.0)))
    val rdd = sparkContext.parallelize(data)
    ...
    assert(result contains 2)
    assert(
      result(1) == MinMaxBounds2D(SingleDimMinMaxBound(-1.8, 2.0), SingleDimMinMaxBound(-1.1, 8.0))
    )
    assert(
      result(2) == MinMaxBounds2D(SingleDimMinMaxBound(0.0, 0.0), SingleDimMinMaxBound(-1.0, 18.0))
    )
  }
