...
import io.github.jakipatryk.sparkpersistenthomology.internal.utils.SingleDimMinMaxBound
...
class PersistenceImageSpec extends AnyFlatSpec with BeforeAndAfterAll {
  ...
  val birthDeathPairs: List[PersistencePair] = List(
    ...
    PersistencePair(1, 1.2f, 11.2f),
    PersistencePair(1, 2.3f, 5.0f),
    PersistencePair(1, 12.1f, 12.5f)
  )
  ...
  it should
    "output persistence image close in values to ones calculated by scikit-tda with the same settings (single dim)" in {
      ...
      val fromScikitTDA =
        loadImageVectorFromFile("/persistenceimage/image-single-dim-scikit-tda.csv")
  ...
  it should
    "output persistence image close in values to ones calculated by scikit-tda with the same settings (multi dim)" in {
      ...
      val fromScikitTDA =
        loadImageVectorFromFile("/persistenceimage/image-multi-dim-scikit-tda.csv")
    ...
    val dimensions = Map(
      0 ->
        BirthAndPersistenceBoundsConfig(
          ...
          maxBirth = Some(2.3),
          minPersistence = Some(0.0),
          maxPersistence = Some(18.4)
        )
    )

    val result =
      PersistenceImage.calculatePixelSizesPerDim(rdd, numberOfPixels, numberOfPixels, dimensions)
  ...
  it should "calculate correct pixel size when all bounds are provided by config (single dim, rectangular image)" in {
    ...
    val result = PersistenceImage
      .calculatePixelSizesPerDim(
        rdd,
        numberOfPixelsBirthAxis,
        numberOfPixelsPersistenceAxis,
        dimensions
      )
      ...
      val dimensions = Map(
        ...
        2 ->
          BirthAndPersistenceBoundsConfig(
            minBirth = Some(1.2),
            maxBirth = Some(454.2),
            minPersistence = Some(0.4),
            maxPersistence = Some(10.0)
          )
      )
  ...
  it should "return Success if config has all values that are mutually correct" in {
    val dimensions = Map(
      0 -> BirthAndPersistenceBoundsConfig(
        minBirth = Some(1.0),
        maxBirth = Some(10.0),
        minPersistence = Some(9.0),
        maxPersistence = Some(11.0)
      ),
      2 -> BirthAndPersistenceBoundsConfig(
        minBirth = Some(0.0),
        maxBirth = Some(0.1),
        minPersistence = Some(10.0),
        maxPersistence = Some(11.0)
      )
    )
  ...
  it should "return Failure with correct message if config has all values that are mutually incorrect" in {
    val dimensions = Map(
      ...
      2 -> BirthAndPersistenceBoundsConfig(
        minBirth = Some(0.0),
        maxBirth = Some(0.1),
        minPersistence = Some(999.0),
        maxPersistence = Some(11.0)
      ),
      5 -> BirthAndPersistenceBoundsConfig(
        minBirth = Some(0.6),
        maxBirth = Some(0.1),
        minPersistence = Some(99.0),
        maxPersistence = Some(11.0)
      )
    )
