...
/** Implementation of concatenation of Combinatorial Number Systems up to `maxCombinationSize`.
  ...
  *   Concatenate CNSs up to this, so included are sizes: 1, 2, ..., `maxCombinationSize`
  */
private[sparkpersistenthomology] class CombinatorialNumberSystemOnSteroids(
  ...
  /** A count of all combinations of `combinationElementsSetSize` elements up to
    * `maxCombinationSize`-combination.
    *
    * In other words, if we let n := combinationElementsSetSize and k := maxCombinationSize: (n
    * choose 1) + (n choose 2) + ... + (n choose k)
    */
  ...
  /** Computes combination (actually indices of elements in the combination) for an index in
    * (global, meaning that in concatenated and limited to `maxCombinationSize`) lexicographical
    * order of combinations.
    *
    * For `maxCombinationSize = 1` lexicographical order of combinations is trivial: (0), (1), ...,
    * (combinationElementsSetSize)
    *
    * Then for `maxCombinationSize = 2` it is: (0), (1), ..., (combinationElementsSetSize), (1, 0),
    * (2, 0), (2, 1), (3, 0), ..., (combinationElementsSetSize, combinationElementsSetSize - 1)
    *
    * And so on.
    *
    * @example
  ...
  def getCombinationFromGlobalIndexInLexicographicalOrder(index: Long): Array[Int] = {
    ...
    val correspondingCombinationSize = getCombinationSizeFromGlobalIndexInLexicographicalOrder(
      index
    )
  ...
  /** Returns an iterator of combinations in global lexicographical order starting with one with
    * index `startIndex`.
    */
...
private[sparkpersistenthomology] object CombinatorialNumberSystemOnSteroids {

  def apply(
    combinationElementsSetSize: Int,
    maxCombinationSize: Int
  ): CombinatorialNumberSystemOnSteroids =
