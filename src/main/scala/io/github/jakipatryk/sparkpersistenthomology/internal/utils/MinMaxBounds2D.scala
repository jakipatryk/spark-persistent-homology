...
/** Represents minimum and maximum values in a single dimension.
  * @tparam T
  *   type that has implicit Numeric implementation (like Double, Int, etc.)
  */
private[sparkpersistenthomology] case class SingleDimMinMaxBound[T](min: T, max: T)(implicit
  n: Numeric[T]
) {

  /** Merges two SingleDimMinMaxBound such that resulting SingleDimMinMaxBound has min equal to min
    * of the two input mins and max equal to max of the two input maxes.
    */
...
/** Represents minimum and maximum values in each of two dimensions.
  ...
  *   type that has implicit Numeric implementation (like Double, Int, etc.)
  */
private[sparkpersistenthomology] case class MinMaxBounds2D[T](
  xBound: SingleDimMinMaxBound[T],
  yBound: SingleDimMinMaxBound[T]
)(implicit n: Numeric[T]) {

  /** Merges two MinMaxBounds2D such that resulting MinMaxBounds2D has xBound equal to merged
    * xBound's of the two inputs, similarly yBound.
    * @see
    *   [[SingleDimMinMaxBound.merge]]
    */
...
private[sparkpersistenthomology] object MinMaxBounds2D {
  ...
  /** For each key in RDD calculates MinMaxBounds2D of its values.
    * @param rdd
    *   key-value RDD where value is a pair
    * @tparam K
    *   type for key
    * @tparam T
    *   type that has implicit Numeric implementation (like Double, Int, etc.)
    * @return
    *   Map where key is of the same type as in RDD and value is of type MinMaxBounds2D representing
    *   mapping key -> bounds.
    */
  def boundsPerKeyFromRDD[K, T](
    rdd: RDD[(K, (T, T))]
